...
local stationNameLimit = 9
local spaceBetweenStations = 9
...
local radioStations = {
	...
	{
		name = "ХИТ ФМ",
		url = "http://online-hitfm2.tavrmedia.ua/HitFM_Ukr"
	},
	{
		name = "ONLY",
		url = "http://radio.minecraftonly.ru:8000/live"
	},
}
...
local function drawMenu()
	local width = 38 + (3 * 2 + 2) * #radioStations
		...
		if e[5] == 0 then
			...
			elseif action == "О программе" then
				ecs.universalWindow("auto", "auto", 36, 0x262626, true, 
					{"EmptyLine"},
					{"CenterText", ecs.colors.orange, "Radio v1.1"}, 
					{"EmptyLine"},
					...
					{"EmptyLine"},
					{"CenterText", 0xFFFFFF, "Обновил:"},
					{"CenterText", 0xBBBBBB, "VETKOVER"}, 
					{"EmptyLine"},
